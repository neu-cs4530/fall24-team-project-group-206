...
describe('Question Controller', () => {
  ...
  describe('GET /getQuestion', () => {
    it('should return questions filtered by search and order', async () => {
      ...
      const response = await supertest(app).get('/question/getQuestion').query({ order: 'someOrder', search: 'someSearch' });
    ...
    it('should return 500 if fetching questions by order fails', async () => {
      ...
      const response = await supertest(app).get('/question/getQuestion').query({ order: 'someOrder' });
  ...
  describe('GET /getQuestionById/:qid', () => {
    it('should return 500 if there is an error fetching question by ID', async () => {
      jest.spyOn(util, 'fetchAndIncrementQuestionViewsById').mockRejectedValueOnce(new Error('Database error'));

      const response = await supertest(app).get('/question/getQuestionById/507f191e810c19729de860ea').query({ username: 'user1' });
  ...
  describe('POST /addQuestion', () => {
    it('should return 500 if there is an error saving a question', async () => {
      jest.spyOn(util, 'processTags').mockResolvedValueOnce([
        { _id: new mongoose.Types.ObjectId('507f191e810c19729de860ea'), name: 'tag1', description: 'tag1 description' },
      ]);
  ...
  describe('PATCH /editQuestion/:qid/:username', () => {
    ...
    it('should return 500 if there is an error updating the question', async () => {
      jest.spyOn(mongoose.Model, 'findByIdAndUpdate').mockRejectedValueOnce(new Error('Database error'));
  ...
  describe('DELETE /removeQuestion/:qid', () => {
    it('should delete a question successfully', async () => {
      ...
      const response = await supertest(app).delete('/question/removeQuestion/507f191e810c19729de860ea');
    ...
    it('should return 500 if there is an error removing the question', async () => {
      jest.spyOn(mongoose.Model, 'findByIdAndDelete').mockRejectedValueOnce(new Error('Database error'));

      const response = await supertest(app).delete('/question/removeQuestion/507f191e810c19729de860ea');

      expect(response.status).toBe(500);
      expect(response.body.error).toBe('Error removing question');
    });
  });
  ...
  it('should filter questions by askedBy field', async () => {
    ...
    const response = await supertest(app).get('/question/getQuestion').query({ askedBy: 'question1_user' });
  ...
  it('should handle unknown error when editing a question', async () => {
    ...
    const response = await supertest(app).patch('/question/editQuestion/507f191e810c19729de860ea/user1').send({
  ...
  it('should handle error if question ID is missing during deletion', async () => {
    const response = await supertest(app).delete('/question/removeQuestion/');

    expect(response.status).toBe(404); // If a route with missing parameters is called, it should return a 404
  });

  it('should handle unknown error during question deletion', async () => {
    jest.spyOn(mongoose.Model, 'findByIdAndDelete').mockRejectedValueOnce(new Error('Database error'));
  ...
  it('should filter questions when `askedBy` is provided', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toEqual([EXPECTED_QUESTIONS[0]]);
  });
  ...
  it('should return 500 if there is an error while fetching question by ID', async () => {
    ...
    const response = await supertest(app).get('/question/getQuestionById/507f191e810c19729de860ea').query({ username: 'test_user' });
  ...
  it('should return 500 if error occurs while deleting question', async () => {
    jest.spyOn(mongoose.Model, 'findByIdAndDelete').mockRejectedValueOnce(new Error('Database error'));
  ...
  it('should return 500 if an unknown error occurs during question deletion', async () => {
